...
def yemot_ivr():
    ...
    if choice == '1':
        return (
            "id_list_message=t-שים לב, הנך מועבר לפתיחת מערכת תוכן חדשה.&t-יש לבצע את כל הפעולות ולחייג שוב למערכת כאשר יש ברשותך מספר מערכת וסיסמה.&go_to=/123456"
        )

    if choice == '2':
        if not system_number:
            return "read=t-אנא הקש את מספר המערכת ובסיום הקש סולמית=system_number,no,1,15,7,No,Yes"
